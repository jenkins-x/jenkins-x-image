...
jx step create pr regex --regex "^(?m)\s+Image: \"jenkinsxio\/jenkinsx\"\s+ImageTag: \"(.*)\"$" --version ${VERSION} --files jenkins-x-platform/values.yaml --repo https://github.com/jenkins-x/jenkins-x-platform.git
